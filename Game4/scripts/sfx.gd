...
func _build_all() -> void:
	...
	_streams["place"] = _wav(_mix([_tone(0.08, 520.0, 780.0, 0.45, 0.9), _noise(0.04, 0.2, 2.0)]))
	_streams["remove"] = _wav(_tone(0.1, 700.0, 380.0, 0.4, 1.0))
	_streams["go"] = _wav(_arp([523.3, 659.3, 784.0], 0.07, 0.3))
	_streams["saved"] = _wav(_arp([880.0, 1174.7], 0.06, 0.22))
	_streams["gold"] = _wav(_arp([880.0, 1108.7, 1318.5, 1760.0], 0.06, 0.5))
	_streams["eaten"] = _wav(_mix([_tone(0.22, 420.0, 120.0, 0.6, 1.2), _noise(0.12, 0.5, 2.5)]))
	_streams["hole"] = _wav(_tone(0.4, 600.0, 90.0, 0.5, 0.9))
	_streams["hiss"] = _wav(_noise(0.35, 0.45, 1.3))
	_streams["crack"] = _wav(_mix([_noise(0.14, 0.7, 2.5), _thump(0.08, 300.0, 120.0)]))
	_streams["rocket"] = _wav(_mix([_noise(1.2, 0.8, 1.1), _tone(1.2, 90.0, 600.0, 0.35, 0.6)]))
	_streams["alarm"] = _wav(_mix([_tone(0.5, 330.0, 330.0, 0.4, 0.5), _tone(0.5, 262.0, 262.0, 0.4, 0.5)]))
