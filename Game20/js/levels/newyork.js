...
export function buildNewYork(w) {
  ...
  for (const [bx, bz] of [[-15, -15], [15, -15], [-15, 15], [15, 15]]) {
    ...
    const sc = screenTex(256, 160, (g, W, H, t) => { if (w.powerOff) { g.fillStyle = "#0c0e14"; g.fillRect(0, 0, W, H); g.strokeStyle = "#262a36"; g.lineWidth = 8; g.strokeRect(4, 4, W - 8, H - 8); g.fillStyle = Math.sin(t * 3) > 0 ? "#6a1a1a" : "#2a0e0e"; g.font = "800 22px system-ui"; g.textAlign = "center"; g.textBaseline = "middle"; g.fillText("⚡ NO POWER", W / 2, H / 2); return; } const k = Math.floor(t * 0.5 + screens.length) % 3; g.fillStyle = ["#e8302a", "#1a4ad8", "#18181c"][k]; g.fillRect(0, 0, W, H); g.fillStyle = k === 2 ? "#ffd166" : "#fff"; g.font = "900 40px system-ui"; g.textAlign = "center"; g.textBaseline = "middle"; g.fillText(txt, W / 2, H / 2 + Math.sin(t * 2) * 6); });
  ...
  w.missionData = {
    ...
    ny2: { cells: [[40, 9.3, 40], [49, 12.3, 40], [49, 15.3, 49], [40, 9.4, 49], [35.6, 6, 40], [44.5, 5, 36], [49, 17, 49], [40, 12, 44.5]] },
