...
func _layout() -> void:
	...
	var w := _size.x
	var h := _size.y
	_fire_rect = Rect2(w - 2.0 * FIRE_R - 34.0, h - 2.0 * FIRE_R - 34.0, 2.0 * FIRE_R, 2.0 * FIRE_R)
	_brake_rect = Rect2(w - 2.0 * FIRE_R - 34.0 - 24.0 - 200.0, h - 130.0 - 34.0, 200.0, 130.0)
...
func _on_down(finger: int, pos: Vector2) -> void:
	...
	if _brake_rect.grow(10.0).has_point(pos):
		...
		_canvas.queue_redraw()
		return
	if pos.x < _size.x * 0.5 and _steer_finger < 0:
		...
		_canvas.queue_redraw()
		return
	# Right-side touches away from the buttons also steer, for players who
	# hold the iPad differently.
	if _steer_finger < 0 and pos.y < _brake_rect.position.y - 20.0:
		_steer_finger = finger
		_steer_origin = pos
		_steer_pos = pos
		_canvas.queue_redraw()
