# Pooled cube debris drawn with a single MultiMesh, so shattering a hundred
# blocks at once still costs one draw call.
class_name Debris
...
func _ready() -> void:
	...
	# Debris is decorative and already expressed in world space.
	top_level = true
...
func _process(delta: float) -> void:
	...
	for i in CAPACITY:
		...
		var s: float = _size[i] * clampf(l, 0.0, 1.0)
		var b := Basis()
		b = b.rotated(Vector3.UP, _spin[i] * l)
		b = b.rotated(Vector3.RIGHT, _spin[i] * l * 0.7)
		mm.set_instance_transform(i, Transform3D(b.scaled(Vector3.ONE * s), _pos[i]))
	_live = still_live
