...
SCENES.amsterdam = function () {
  ...
  const bike = (x, z, ry, col) => {
    const g = new THREE.Group(); g.position.set(x, 0, z); g.rotation.y = ry; S.add(g);
    const t = this.M({ color: 0x141414 }), fm = this.M({ color: col, metalness: 0.4, roughness: 0.5 });
    const bar = (w, h, d, px, py, pz, rx, m) => { const b = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), m || fm); b.position.set(px, py, pz); if (rx) b.rotation.x = rx; b.castShadow = true; g.add(b); return b; };
    for (const dz of [-0.62, 0.62]) {
      const w = new THREE.Mesh(new THREE.TorusGeometry(0.33, 0.045, 6, 16), t); w.rotation.y = Math.PI / 2; w.position.set(0, 0.33, dz); w.castShadow = true; g.add(w);
      const hub = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.07, 8), t); hub.rotation.z = Math.PI / 2; hub.position.set(0, 0.33, dz); g.add(hub);
      for (let k = 0; k < 4; k++) { const sp = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.62, 0.02), t); sp.position.set(0, 0.33, dz); sp.rotation.x = k * Math.PI / 4; g.add(sp); }
    }
    bar(0.05, 0.05, 0.92, 0, 0.66, 0.06, 0.22);          // top tube
    bar(0.05, 0.05, 0.78, 0, 0.44, -0.12, -0.62);        // down tube
    bar(0.05, 0.5, 0.05, 0, 0.6, 0.34);                  // seat tube
    bar(0.05, 0.05, 0.62, 0, 0.5, 0.48, 0.95);           // chain stay
    bar(0.05, 0.62, 0.05, 0, 0.62, -0.5);                // fork and steerer
    bar(0.26, 0.09, 0.1, 0, 0.88, 0.36, 0, t);           // saddle
    const hb = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.04, 0.04), t); hb.position.set(0, 0.96, -0.5); g.add(hb);
    const basket = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.24, 0.26), this.M({ color: 0x8a6a3a, roughness: 1 })); basket.position.set(0, 0.8, -0.62); basket.castShadow = true; g.add(basket);
    return g;
  };
  ...
  // ambient life
  this.crowd([[2, 6], [16, 6], [16, 10], [2, 10]], 4, { speed: 1.0, hat: "cap" });
  ...
  return { x: -8, z: 3, yaw: 0.2, bounds: { x0: -32, x1: 34, z0: -4, z1: 12 } };
};
