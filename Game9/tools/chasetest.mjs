...
ck(chases.length === 3, `three chase missions (${chases.length})`);
for (const r of chases) {
  ...
  ck(await ev(() => !!__spy.mg.root && __spy.mg.root.children.length > 20), `${r.id} builds a road and scenery`);
  ...
  ck(a.eye > 1 && a.eye < 1.7, `${r.id} rides at a sensible height (${a.eye})`);
  ...
  ck(lat1 > lat0 + 0.8, `${r.id} steering moves it across the road (${lat0.toFixed(1)} -> ${lat1.toFixed(1)})`);
  ...
  await ev(() => { const mg = __spy.mg; const h = mg.haz[0]; h.s = mg.s + 0.2; h.lat = mg.lat; h.hitT = 0; });
  await waitGame(0.5);
