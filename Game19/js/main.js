// Agent Rory: Meltdown — the game loop, input, HUD and mission flow.
...
function loadScene(id) { G.world.foundBugs = new Set(G.save.bugs || []); return G.world.load(id, COUNTRIES.find(c => c.id === id)); }
...
// runs build their own world, so no mini-game needs its city to be loaded
const NEEDS_WORLD = [];
...
function showHint() { if (G.mg && G.mg.hint) { const h = G.mg.hint(); if (h) { G.mg.hintsUsed++; G.hint = { text: h, t: 5 }; Speech.say(h, CHARS.pip.voice, null); } } }
...
function draw(dt) {
  ...
  if (G.hint) { const a = clamp(G.hint.t, 0, 1); g.globalAlpha = a; panel(g, W / 2 - 320 * s, 70 * s, 640 * s, 80 * s, s, { bg: "rgba(90,42,138,.92)", border: "#c9a1ff" }); UI.drawPortrait(g, "pip", W / 2 - 310 * s, 76 * s, 68 * s, 0.4, G.t); UI.paragraph(g, G.hint.text, W / 2 - 230 * s, 96 * s, 540 * s, 17 * s, "#fff", 21 * s, "left", 600); g.globalAlpha = 1; }
  if (G.card) {
